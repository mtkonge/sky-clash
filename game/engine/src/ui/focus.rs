...
impl Focus {
    ...
    pub fn update(&mut self, dom: &mut ui::Dom, ctx: &mut crate::Context) {
        let gamepad_next = ctx.active_controllers().any(|id| {
            ctx.controller_button_just_pressed(id, crate::ControllerButton::DPadRight)
                || ctx.controller_button_just_pressed(id, crate::ControllerButton::DPadDown)
        });
        let gamepad_previous = ctx.active_controllers().any(|id| {
            ctx.controller_button_just_pressed(id, crate::ControllerButton::DPadLeft)
                || ctx.controller_button_just_pressed(id, crate::ControllerButton::DPadUp)
        });
        let gamepad_confirm = ctx
            .active_controllers()
            .any(|id| ctx.controller_button_just_pressed(id, crate::ControllerButton::A));
        let cycle_next = (ctx.key_just_pressed(crate::Keycode::Tab)
            && !ctx.key_pressed(crate::Keycode::LShift))
            || gamepad_next;
        let cycle_previous = (ctx.key_just_pressed(crate::Keycode::Tab)
            && ctx.key_pressed(crate::Keycode::LShift))
            || gamepad_previous;
        if cycle_next {
            self.next(dom);
        ...
        if ctx.key_just_pressed(crate::Keycode::Return) || gamepad_confirm {
